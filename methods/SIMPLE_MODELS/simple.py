...
parser.add_argument('--repo_path', default = '/home/emiliano/projects/def-cbravo/emiliano/RappiAnomalyDetection/methods/GNN_LSTM/',type = str,
                    help='to run LSTM /GRU on batch or for loop basis')                    
# dataset name
...
def train(data_dict,ts_list,model_GAT, model_RNN, model_FFNN, n_epoch,lr,\
    ...
    val_h0_test = False, upsample = False,ONLY_RNN = False,train_forward_funcion = None,no_windows = False ,boot_sample = 10):
    ...
    print(f' VAL AUC at epoch {best_epoch}:')
    out_val_auc = mean(val_auc)
    print(mean(val_auc))
...
def get_data_node_classification(dataset_name, GCN,full_batch,base,page_rank = False):
    ...
    if page_rank:
        datas_pagerank = torch.load('/home/emiliano/projects/def-cbravo/emiliano/RappiAnomalyDetection/methods/GNN_LSTM/data/TGN_paper_PR.pt')
        datas  = datas_pagerank
        print('PAGERANK DATA')
    elif GCN == 'GAT': 
        datas_GAT = torch.load('/home/emiliano/projects/def-cbravo/emiliano/RappiAnomalyDetection/methods/GNN_LSTM/data/TGN_paper_GAT.pt')
        datas = datas_GAT
        print('GAT DATA')
    elif GCN == 'GCN': 
        datas_GCN = torch.load('/home/emiliano/projects/def-cbravo/emiliano/RappiAnomalyDetection/methods/GNN_LSTM/data/TGN_paper_GCN.pt')
        datas = datas_GCN
        print('GCN OR FEATURES DATA')
...
class GAT(torch.nn.Module):
    def __init__(self, dim_in, dim_h, dim_out, edge_dim,num_heads,num_layers,dropout_rate):
        super().__init__()
        self.dropout_rate = dropout_rate
        self.gat1 = GATv2Conv(dim_in, dim_h, heads=num_heads,edge_dim = edge_dim) # dim_h * num heads
        #dim_h * heads > dim_h
        self.GAT_list = torch.nn.ModuleList([GATv2Conv(dim_h*num_heads, dim_h, heads=num_heads,edge_dim = edge_dim)  for _ in range(num_layers)])
        self.gat2 = GATv2Conv(dim_h*num_heads, dim_out, heads=1, edge_dim = edge_dim)
...
if args.GNN == 'GAT':
    model_GAT = GAT(number_feats, num_hidden, num_embeddings,edge_dim = data_dict[ts_list[0]].edge_attr.shape[1],num_heads = num_heads ,num_layers=n_layers, dropout_rate=dropout_rate)
elif args.GNN == 'GCN': 
    model_GAT = GCN(number_feats, num_hidden, num_embeddings,num_layers=n_layers, dropout_rate=dropout_rate)
if args.RNN == 'GRU':
...
n_epoch=n_epoch,lr = lr,momentum = momentum, sample_rate =sample_rate,\
    no_windows=wind,upsample = upsample, train_forward_funcion=train_foward_function, boot_sample = args.boot_sample)
...
p = '/home/emiliano/projects/def-cbravo/etiukhov/Journal_paper'
path = p+ f'/final_models/{args.run}'

isExist = os.path.exists(path)
log_file = p+'/logs/log_df/logs_simple_correct_data_final.csv'
