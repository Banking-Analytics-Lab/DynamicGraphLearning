...
def get_data_node_classification(dataset_name, GCN,full_batch):
### Load data and train val test split
    #change path
    base = '/home/emiliano/projects/def-cbravo/emiliano/RappiAnomalyDetection/methods/TGN/tgn/'
...
print(args)
data_dict,ts_list = get_data_node_classification('TGN_paper',args.GNN,args.full_batch)
# num_hidden = trial.suggest_int('num_hidden',100,200,50)
# num_embeddings = trial.suggest_int('num_embeddings',100,200,50)
# n_layers = trial.suggest_int('n_layers',3,5,2)
# lr = trial.suggest_float('lr',.1,.3)
# momentum = trial.suggest_float('momentum',.009,.09)
# sample_rate = trial.suggest_float('sample_rate',1,1.2)
# print(f'{num_embeddings=} , {num_hidden=}, {n_layers=}, {lr=}, {momentum=}, {sample_rate=}')
...
if args.GNN == 'GAT':
    model_GAT = GAT(25, num_hidden, num_embeddings,edge_dim = data_dict[ts_list[0]].edge_attr.shape[1],heads = 2 ,num_layers=n_layers)
elif args.GNN == 'GCN': 
    model_GAT = GCN(25, num_hidden, num_embeddings,num_layers=n_layers)
if args.RNN == 'GRU':
...
#change path
path = f'/home/emiliano/projects/def-cbravo/emiliano/RappiAnomalyDetection/models_final/{args.run}'

isExist = os.path.exists(path)
#change path 
log_file = '/home/emiliano/projects/def-cbravo/emiliano/RappiAnomalyDetection/logs/log_df/logs_hp_new.csv'
df_cols =['model',
 'train_time',
 'VAL_AUC_SEEN',
 'VAL_AUC_UNSEEN',
 ...
 'TRAIN_AUC',
 'TRAIN_AUPRC']
print(len(metrics),len(df_cols))
rows =[args.run] + [total] + metrics 

df = pd.DataFrame({c:[r] for c,r in zip(df_cols,rows)})
